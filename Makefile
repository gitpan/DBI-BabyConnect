# This Makefile is for the DBI::BabyConnect extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/DBI/BabyConnect.pm]
#     AUTHOR => q[Bassem W. Jamaleddine <bassem@tcnd.com>]
#     NAME => q[DBI::BabyConnect]
#     PREREQ_PM => { DBI=>q[1.5] }
#     VERSION_FROM => q[lib/DBI/BabyConnect.pm]
...
DIRFILESEP = /
DFSEP = $(DIRFILESEP)
NAME = DBI::BabyConnect
NAME_SYM = DBI_BabyConnect
VERSION = 0.92
...
FULLEXT = DBI/BabyConnect
BASEEXT = BabyConnect
PARENT_NAME = DBI
DLBASE = $(BASEEXT)
VERSION_FROM = lib/DBI/BabyConnect.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/DBI/BabyConnect.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/DBI
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/DBI
...
TO_INST_PM = lib/DBI/BabyConnect.pm

PM_TO_BLIB = lib/DBI/BabyConnect.pm \
...
DISTNAME = DBI-BabyConnect
DISTVNAME = DBI-BabyConnect-0.92
...
manifypods : pure_all  \
	lib/DBI/BabyConnect.pm \
	lib/DBI/BabyConnect.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/DBI/BabyConnect.pm $(INST_MAN3DIR)/DBI::BabyConnect.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:         DBI-BabyConnect' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:      0.92' >> META_new.yml
	$(NOECHO) $(ECHO) 'version_from: lib/DBI/BabyConnect.pm' >> META_new.yml
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>creates an object that holds a DBI connection to a database</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Bassem W. Jamaleddine &lt;bassem@tcnd.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="DBI" VERSION="1,5,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' \
	  lib/DBI/BabyConnect.pm blib/lib/DBI/BabyConnect.pm 
